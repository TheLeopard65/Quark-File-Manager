...
struct OPTIONS {
  ...
  bool show_new_file_dialog = false;
  bool show_properties_dialog = false;
  bool dialog_has_focus = false;
};
...
#endif
  OPTIONS options;
  std::string bookmarks[8] = {options.path,
...
void left_window(std::string &path, std::string *bookmarks, ImGuiWindowFlags bookmarks_window_flags) {
  ...
  if (ImGui::BeginChild("Bookmarks", ImVec2(320, 0), true, bookmarks_window_flags)) {
    for (int i = 0; i < 8; i++) {
  ...
  if (ImGui::BeginChild("File Manager", ImVec2(0, 0), true, file_manager_window_flags)) {
    ...
    if (ImGui::BeginTable("file_table", 5, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingStretchSame)) {
      ...
      for (auto &i : std::filesystem::directory_iterator(options.path)) {
        ...
        std::string size_str = "-";
        if (!is_dir)
          size_str = std::to_string(std::filesystem::file_size(i.path())) + "B";
    ...
    if (options.is_app_options) {
      ...
      if (ImGui::IsMouseClicked(0) &&
          (ImGui::GetMousePos().x < options.mouse_pos.x ||
           ImGui::GetMousePos().x > options.mouse_pos.x + 200 ||
           ImGui::GetMousePos().y < options.mouse_pos.y ||
           ImGui::GetMousePos().y > options.mouse_pos.y + 200)) {
        options.is_app_options = false;
      }
      ImGui::SetNextWindowPos(options.mouse_pos);
      ImGui::SetNextWindowSize(ImVec2(275, 250));
      ...
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
        options.show_new_folder_dialog = true;
        options.dialog_has_focus = true;
        options.new_folder_name[0] = '\0';
        options.is_app_options = false;
      }
      ImGui::Selectable("New file", false, 0, ImVec2(0, 40));
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
        options.show_new_file_dialog = true;
        options.dialog_has_focus = true;
      ...
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
        options.show_rename_dialog = true;
        options.dialog_has_focus = true;
        strncpy(options.new_file_name, std::filesystem::path(options.clicked_path).filename().c_str(), 100);
        options.is_app_options = false;
      }
      ...
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
        options.show_properties_dialog = true;
#ifdef _WIN32
        std::string command = "exiftool \"" + options.clicked_path + "\"";
#else
        std::string command = "exiftool \"" + options.clicked_path + "\" 2>&1";
#endif
        options.properties_output = exec(command.c_str());
        if (options.properties_output.empty()) {
          options.properties_output = "No properties available\nInstall \"exiftool\" for detailed file information";
        }
        options.is_app_options = false;
      }
      ImGui::End();
    ...
    if (options.is_dir_options) {
      ...
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0) &&
        ...
        if (std::filesystem::is_directory(options.copy_path)) {
          ...
          std::system(
              ("cp " + options.copy_path + " " + options.clicked_path).c_str());
          if (options.is_cut)
            std::system(("rm " + options.copy_path).c_str());
        }
        options.copy_path = "";
    ...
    if (options.show_rename_dialog) {
      ImGui::OpenPopup("Rename");
      options.show_rename_dialog = false;
    }
    if (ImGui::BeginPopupModal("Rename", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
      if (options.dialog_has_focus) {
        ImGui::SetKeyboardFocusHere();
        options.dialog_has_focus = false;
      }
      bool enterPressed = ImGui::InputText("##rename", options.new_file_name, 100, ImGuiInputTextFlags_EnterReturnsTrue);
      if (ImGui::Button("Confirm") || enterPressed) {
        auto old_path = std::filesystem::path(options.clicked_path);
        auto new_path = old_path.parent_path() / options.new_file_name;
    ...
    if (options.show_new_folder_dialog) {
      ImGui::OpenPopup("New Folder");
      options.show_new_folder_dialog = false;
    }
    if (ImGui::BeginPopupModal("New Folder", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
      ...
      bool enterPressed = ImGui::InputText("##newfolder", options.new_folder_name, 100, ImGuiInputTextFlags_EnterReturnsTrue);
      if (ImGui::Button("Confirm") || enterPressed) {
        auto new_dir = std::filesystem::path(options.path) / options.new_folder_name;
    ...
    if (options.show_new_file_dialog) {
      ImGui::OpenPopup("New File");
      options.show_new_file_dialog = false;
    }
    if (ImGui::BeginPopupModal("New File", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
      ...
      bool enterPressed = ImGui::InputText("##newfile", options.new_file_name, 100, ImGuiInputTextFlags_EnterReturnsTrue);
      if (ImGui::Button("Confirm") || enterPressed) {
        auto new_file = std::filesystem::path(options.path) / options.new_file_name;
        std::ofstream(new_file.string()).close();
        ImGui::CloseCurrentPopup();
      }
      ImGui::SameLine();
      if (ImGui::Button("Cancel")) {
    ...
    if (options.show_properties_dialog) {
      ImGui::OpenPopup("Properties");
      options.show_properties_dialog = false;
    }
    if (ImGui::BeginPopupModal("Properties", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize)) {
      ImGui::BeginChild("PropertiesScroll", ImVec2(1440, 720), true);
      ImGui::TextUnformatted(options.properties_output.c_str());
      ImGui::EndChild();
